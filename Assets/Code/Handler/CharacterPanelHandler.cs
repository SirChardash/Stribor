...
namespace Code.Handler
{
  public class CharacterPanelHandler : MonoBehaviour
  {
    ...
    private void GenerateButtons(Character character)
    {
      ...
      for (var i = 0; i < character.Behavior.AvailableActions.Count; i++)
      {
        ...
        {
          ActiveCharacter.Order.Action = character.Behavior.AvailableActions[i1];
          InputState.Current = InputState.State.ChooseTarget;
        });
        button.GetComponentInChildren<Text>().text = character.Behavior.AvailableActions[i].GetType().Name;
        _buttons.Add(button);
      }
    }
  }
